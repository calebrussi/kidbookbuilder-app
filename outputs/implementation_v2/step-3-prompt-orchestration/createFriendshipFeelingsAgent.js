...
async function createAgent() {
  try {
    console.log('Starting friendship & feelings agent creation process...');
    ...
    // Delete any existing friendship & feelings agents
    await deleteExistingAgents('friendship_feelings_agent');

    // Read the agent configuration
    const agentConfigPath = path.join(__dirname, 'agents', 'friendship_feelings_agent.json');
    ...
    console.log('Friendship & Feelings Agent created successfully!');
    console.log('Agent ID:', response.data.agent_id);
    console.log('Agent Status:', response.data.status);
    
    // Save the agent details to a file for future reference
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const outputPath = path.join(__dirname, 'agents', `friendship_feelings_agent_${timestamp}.json`);
