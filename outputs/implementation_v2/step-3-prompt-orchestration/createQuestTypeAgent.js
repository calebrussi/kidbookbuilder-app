...
async function createAgent() {
  try {
    console.log('Starting quest type agent creation process...');
    ...
    // Delete any existing quest type agents
    await deleteExistingAgents('quest_type_agent');

    // Read the quest type agent configuration
    const agentConfigPath = path.join(__dirname, 'agents', 'quest_type_agent.json');
    ...
    console.log('Quest Type Agent created successfully!');
    console.log('Agent ID:', response.data.agent_id);
    console.log('Agent Status:', response.data.status);
    
    // Save the agent details to a file for future reference
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const outputPath = path.join(__dirname, 'agents', `quest_type_agent_${timestamp}.json`);
