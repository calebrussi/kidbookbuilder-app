...
async function createAgent() {
  try {
    ...
    // Read the personality agent configuration
    const agentConfigPath = path.join(__dirname, 'agents', 'personality_agent_beta.json');
    ...
    console.log('Personality Agent created successfully!');
    console.log('Agent ID:', response.data.agent_id);
    console.log('Agent Status:', response.data.status);
    
    // Save the agent details to a file for future reference
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const outputPath = path.join(__dirname, 'agents', `personality_agent_${timestamp}.json`);
