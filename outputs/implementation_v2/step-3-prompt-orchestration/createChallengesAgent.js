...
async function createAgent() {
  try {
    console.log('Starting challenges to face agent creation process...');
    ...
    // Delete any existing challenges to face agents
    await deleteExistingAgents('challenges_to_face_agent');

    // Read the agent configuration
    const agentConfigPath = path.join(__dirname, 'agents', 'challenges_to_face_agent.json');
    ...
    console.log('Challenges Agent created successfully!');
    console.log('Agent ID:', response.data.agent_id);
    console.log('Agent Status:', response.data.status);
    
    // Save the agent details to a file for future reference
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const outputPath = path.join(__dirname, 'agents', `challenges_agent_${timestamp}.json`);
