...
async function createAgent() {
  try {
    console.log('Starting supporting characters agent creation process...');
    ...
    // Read the supporting characters agent configuration
    const agentConfigPath = path.join(__dirname, 'agents', 'supporting_characters_beta.json');
    ...
    console.log('Supporting Characters Agent created successfully!');
    console.log('Agent ID:', response.data.agent_id);
    console.log('Agent Status:', response.data.status);
    
    // Save the agent details to a file for future reference
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const outputPath = path.join(__dirname, 'agents', `supporting_characters_agent_${timestamp}.json`);
