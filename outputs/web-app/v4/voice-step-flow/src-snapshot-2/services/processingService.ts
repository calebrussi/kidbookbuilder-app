...
class ProcessingService {
  ...
  async fetchConversationAndUpdateProgress(
    ...
    //   const stepStatus:StepStatus = resp.data.analysis?.call_successful || progress.stepProgress[progress.currentStepId].status;
      ...
      const updatedProgress = JSON.parse(JSON.stringify(progress)) as UserProgress;
      ...
      if (success === true) {
        console.log(`Marking step ${progress.currentStepId} as complete in processing service`);
        updatedProgress.stepProgress[progress.currentStepId].status = 'complete';
        updatedProgress.stepProgress[progress.currentStepId].completedAt = new Date();
      ...
      if (handlers?.onConversationProgressUpdate) {
        ...
        handlers.onConversationProgressUpdate(
          updatedProgress.currentStepId, 
          { analysis, success, conversationStatus }
        );
      }
