import { UserProgress, StepProgress, StepStatus, CapturedData, ConversationResult, Message } from '../types/userProgress';
...
class ProgressService {
  ...
  updateStepConversationId(progress: UserProgress, stepId: string, conversationId: string): UserProgress {
    ...
    console.log(`Updated conversation ID for step ${stepId}: ${conversationId}`);
  ...
  updateConversationResult(progress: UserProgress, stepId: string, conversationResult: ConversationResult): UserProgress {
    const updatedProgress = { ...progress };
    
    if (!updatedProgress.stepProgress[stepId]) return progress;

    // Update the conversation result for the specific step
    updatedProgress.stepProgress[stepId] = {
      ...updatedProgress.stepProgress[stepId],
      conversationResult,
      lastModified: new Date()
    ...
    // If the conversation was successful and not already complete, mark step as complete
    if (conversationResult.success && updatedProgress.stepProgress[stepId].status !== 'complete') {
      updatedProgress.stepProgress[stepId].status = 'complete';
      updatedProgress.stepProgress[stepId].completedAt = conversationResult.completedAt;
    }

    // If the conversation failed, mark step as error
    if (!conversationResult.success) {
      updatedProgress.stepProgress[stepId].status = 'error';
    }
  ...
  addMessage(progress: UserProgress, stepId: string, message: Message): UserProgress {
    const updatedProgress = { ...progress };
    ...
    updatedProgress.stepProgress[stepId] = {
      ...updatedProgress.stepProgress[stepId],
      messages: [...updatedProgress.stepProgress[stepId].messages, message],
      lastModified: new Date()
    };
